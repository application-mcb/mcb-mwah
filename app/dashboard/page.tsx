  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        // Fetch subjects and subject sets data for enrolled students
        try {
  ...
  const handleEditSuccess = async () => {
    ...
    if (user) {
      try {
        ...
        // Also refresh subjects and subject sets data
        try {
          const [subjectsResponse, subjectSetsResponse] = await Promise.all([
            fetch('/api/subjects'),
            fetch('/api/subject-sets')
  ...
  useEffect(() => {
    ...
    const subjectsInterval = setInterval(() => {
      ...
      if (studentSubjects.length > 0) {
        setSubjectsCarouselIndex((prev) => (prev + 1) % Math.ceil(studentSubjects.length / 3));
      }
    }, 4000);

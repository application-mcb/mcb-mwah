...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.info('Courses fetched successfully', { count: courses.length, search });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { code, name, description, color, registrarUid } = await request.json();

    // Validate required fields
    if (!code || !name || !description || !color || !registrarUid) {
      return NextResponse.json(
        { error: 'Missing required fields: code, name, description, color, registrarUid' },
        { status: 400 }
      );
    }
    ...
    if (!/^[A-Z0-9]{2,10}$/.test(sanitizedCode)) {
      return NextResponse.json(
        { error: 'Invalid course code format. Must be 2-10 alphanumeric characters.' },
        { status: 400 }
      );
    }
    ...
    logger.info('Course created successfully', { code: course.code, createdBy: registrarUid });
    return NextResponse.json({
      course,

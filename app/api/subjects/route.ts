...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (search) {
      ...
      subjects = await SubjectDatabase.getSubjectsByGradeLevel(
        parseInt(gradeLevel)
      )
    } else if (courseCode) {
      ...
      if (SUBJECT_COLORS.includes(color as SubjectColor)) {
        subjects = await SubjectDatabase.getSubjectsByColor(
          color as SubjectColor
        )
      } else {
    ...
    return NextResponse.json(
      {
        error: errorMessage,
        details: { gradeLevel, courseCode, color, search },
      },
      { status: 500 }
    )
  }
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            'Missing required fields: code, name, description, color, lectureUnits, labUnits, registrarUid',
        },
        { status: 400 }
      )
    }
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            'At least one grade level, grade ID, or college course must be selected',
        },
        { status: 400 }
      )
    }
    ...
    const subjectData: CreateSubjectData = {
      ...
      gradeLevels: gradeLevels
        ? Array.from(new Set(gradeLevels.map((level: any) => parseInt(level))))
        : [],
      gradeIds: gradeIds
        ? Array.from(new Set(gradeIds.map((id: any) => String(id))))
        : [],
    ...
    try {
      await AuditLogDatabase.createLog({
        ...
        category: 'subjects',
        status: 'success',
        context: `${subjectData.code} • ${subjectData.name}`,
        ...
        metadata: {
          ...
          gradeIds: subjectData.gradeIds,
          courseCodes: subjectData.courseCodes,
          courseSelections: subjectData.courseSelections,
          lectureUnits: subjectData.lectureUnits,
          labUnits: subjectData.labUnits,
          totalUnits: subjectData.totalUnits,
          prerequisites: subjectData.prerequisites,
          postrequisites: subjectData.postrequisites,
          color: subjectData.color,
        },
      })
    } catch (logError) {
      console.warn('Audit log write failed (subject create):', logError)
    }

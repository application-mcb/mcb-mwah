...
import { Calculator, X, Plus, Trash, Books, GraduationCap, Check } from '@phosphor-icons/react';
...
export default function SubjectAssignmentForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const selectedSubjectSet = subjectSets.find(set => set.id === formData.subjectSetId);
    ...
    <div className="bg-white shadow-lg max-w-4xl w-full p-6">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600" style={{ fontFamily: 'Poppins', fontWeight: 300 }}>
              {isEditing ? 'Update subject assignment information' : 'Assign subject sets to grade levels or courses'}
            </p>
          </div>
        </div>
        <button
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          <Label className="block text-sm font-medium text-gray-700" style={{ fontFamily: 'Poppins', fontWeight: 400 }}>
            Level *
          </Label>
          ...
          {errors.level && (
            <p className="text-sm text-red-600" style={{ fontFamily: 'Poppins', fontWeight: 300 }}>
              {errors.level}
            </p>
          )}
        </div>
          ...
          <div className="space-y-2">
            ...
            {formData.gradeLevel && (
              <p className="text-xs text-gray-600" style={{ fontFamily: 'Poppins', fontWeight: 300 }}>
                Selected: {(() => {
                  const selectedGrade = grades.find(g => g.gradeLevel === formData.gradeLevel);
                  if (selectedGrade) {
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  style={{
                    fontFamily: 'Poppins',
                    fontWeight: 300,
                    backgroundColor: formData.semester === 'first-sem' ? getCourseColorValue(getCourseColor(formData.courseCode)) : 'white',
                    borderColor: formData.semester === 'first-sem' ? getCourseColorValue(getCourseColor(formData.courseCode)) : '#d1d5db',
                    color: formData.semester === 'first-sem' ? 'white' : '#374151'
                  }}
                >
                  First Semester
                </button>
                <button
                  ...
                  style={{
                    fontFamily: 'Poppins',
                    fontWeight: 300,
                    backgroundColor: formData.semester === 'second-sem' ? getCourseColorValue(getCourseColor(formData.courseCode)) : 'white',
                    borderColor: formData.semester === 'second-sem' ? getCourseColorValue(getCourseColor(formData.courseCode)) : '#d1d5db',
                    color: formData.semester === 'second-sem' ? 'white' : '#374151'
                  }}
                >
                  Second Semester
                </button>
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2 max-h-60 overflow-y-auto p-2 border border-gray-200 rounded-md bg-gray-50">
            {subjectSets.map(subjectSet => {
              ...
              const setColorValue = getSubjectSetColorValue(subjectSet.color || 'blue-800');
              return (
                <button
                  key={subjectSet.id}
                  type="button"
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs opacity-80">({subjectSet.subjects.length})</span>
                  </div>
                </button>
              );
            })}
          ...
          {formData.subjectSetId && (
            <p className="text-xs text-gray-600" style={{ fontFamily: 'Poppins', fontWeight: 300 }}>
              Selected: {subjectSets.find(s => s.id === formData.subjectSetId)?.name || 'Unknown'}
            </p>
          )}
          {errors.subjectSetId && (
            <p className="text-sm text-red-600" style={{ fontFamily: 'Poppins', fontWeight: 300 }}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-900" style={{ fontFamily: 'Poppins', fontWeight: 400 }}>
                      {formData.level === 'high-school' ? 'High School' : 'College'}
                    </p>
                  </div>
                  {formData.level === 'high-school' ? (
                    <div>
                    ...
                    <>
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium text-gray-900" style={{ fontFamily: 'Poppins', fontWeight: 400 }}>
                          {formData.semester === 'first-sem' ? 'First Semester' : 'Second Semester'}
                        </p>
                      </div>
                    </>
                  )}
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-900" style={{ fontFamily: 'Poppins', fontWeight: 400 }}>
                      {subjectSets.find(s => s.id === formData.subjectSetId)?.name || 'Unknown'}
                    </p>
                  </div>
                </div>
              </div>
                    ...
                    <tfoot className="bg-gray-50 border-t-2 border-gray-300">
                      <tr>
                        ...
                        <td className="px-4 py-3 text-sm font-medium text-gray-900 text-center" style={{ fontFamily: 'Poppins', fontWeight: 500 }}>
                          {selectedSubjectDetails.reduce((sum, s) => sum + (s.lectureUnits || 0), 0)}
                        </td>
                        <td className="px-4 py-3 text-sm font-medium text-gray-900 text-center" style={{ fontFamily: 'Poppins', fontWeight: 500 }}>
                          {selectedSubjectDetails.reduce((sum, s) => sum + (s.labUnits || 0), 0)}
                        </td>
                        <td className="px-4 py-3 text-sm font-medium text-gray-900 text-center" style={{ fontFamily: 'Poppins', fontWeight: 500 }}>
                          {selectedSubjectDetails.reduce((sum, s) => sum + (s.lectureUnits || 0) + (s.labUnits || 0), 0)}
                        </td>
                      </tr>
                    </tfoot>
                  </table>

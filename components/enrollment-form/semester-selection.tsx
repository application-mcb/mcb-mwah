...
export default function SemesterSelectionStep({
  ...
  const currentSemesterFormat =
    currentSystemSemester === '1' ? 'first-sem' : currentSystemSemester === '2' ? 'second-sem' : null
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Choose the semester you wish to enroll in</p>
            </div>
          </div>
        </div>
        ...
        {(['first-sem', 'second-sem'] as const).map((sem) => (
          <Card
            ...
            className={`group p-6 border-none border-1 shadow-sm transition-all duration-300 ${
              !isSemesterSelectable(sem) ? 'cursor-not-allowed' : 'hover:border-blue-900 cursor-pointer hover:shadow-lg bg-gray-50'
          ...
          >
            <div className="space-y-4 flex flex-col justify-between h-full">
                ...
                <div className="flex items-center space-x-3">
                  <div className={`w-10 h-10 flex items-center justify-center bg-white`}>
                    <Calendar
                      size={20}
                      weight="fill"
                      className={`${selectedSemester === sem ? 'text-blue-900' : 'text-blue-800'}`}
                    />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3
                      className={`text-lg font-medium ${selectedSemester === sem ? 'text-white' : 'text-white'}`}
                      style={{ fontFamily: 'Poppins', fontWeight: 500 }}
                    >
                      {sem === 'first-sem' ? 'First Semester' : 'Second Semester'}
                    </h3>
                    <p className={`text-sm ${selectedSemester === sem ? 'text-white' : 'text-white'}`}>
                      {sem === 'first-sem' ? 'August - December' : 'January - May'}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <p className={`text-sm ${selectedSemester === sem ? 'text-white' : 'text-white'}`}>
                {sem === 'first-sem' ? 'First semester enrollment for the academic year.' : 'Second semester enrollment for the academic year.'}
              </p>

              <div className="pt-2 border-t border-gray-200">
                <div className="flex items-center justify-between">
                  <span className={`text-sm ${selectedSemester === sem ? 'text-white' : 'text-white'}`}>
                    {!isSemesterSelectable(sem) ? 'Not available' : 'Click to select'}
                  </span>
                  {!isSemesterSelectable(sem) ? (
                    <WarningCircle size={16} className="text-red-200" weight="bold" />
                  ) : (
                    <div className={`w-4 h-4 border-2 ${selectedSemester === sem ? 'border-white bg-white' : 'border-white'}`}></div>
                  )}
                </div>
              </div>
            </div>

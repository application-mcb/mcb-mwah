...
export default function AcademicRecords({ userId, studentName, onNavigateToEnrollment }: AcademicRecordsProps) {
        ...
        Object.entries(gradesData.grades).forEach(([subjectId, subjectGrade]: [string, any]) => {
          if (subjectGrade && typeof subjectGrade === 'object') {
            ...
            sanitizedGrades[subjectId] = {
              ...
              specialStatus: (subjectGrade.specialStatus === 'INC' || subjectGrade.specialStatus === 'FA' || subjectGrade.specialStatus === 'FW' || subjectGrade.specialStatus === 'W') ? subjectGrade.specialStatus : null
            };
          }
        });
  ...
  const calculateAverage = (period1: number | null, period2: number | null, period3: number | null, period4: number | null, isCollege: boolean = false, specialStatus?: 'INC' | 'FA' | 'FW' | 'W' | null): { average: number | null; completedPeriods: number } => {
    ...
    if (isCollege) {
      ...
      periods = [period1, period2, period3].filter(p => p !== null && typeof p === 'number');
    } else {
      // High School: use all 4 periods
      periods = [period1, period2, period3, period4].filter(p => p !== null && typeof p === 'number');
    }
  ...
  const convertToNumericMode = (percentage: number | null): number | null => {
    if (percentage === null || percentage === undefined || percentage === 0) return null;
  ...
  const getDescriptiveMode = (percentage: number | null): string => {
    if (percentage === null || percentage === undefined || percentage === 0) return 'Incomplete';
  ...
  if (!enrollment || enrollment.enrollmentInfo?.status !== 'enrolled') {
    return (
      <div className="space-y-6">
        ...
        <Card className="p-12 text-center border-none bg-red-50 border-1 shadow-sm border-red-500">
          ...
          <p
            className="text-red-700 text-justify border-1 shadow-sm border-red-500 p-4 bg-red-100 mb-6 max-w-lg mx-auto"
            style={{ fontFamily: 'Poppins', fontWeight: 300 }}
          >
          ...
          <table className="w-full">
              ...
              <tr>
                ...
                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200"
                  ...
                  <div className="flex items-center justify-center gap-2">
                    ...
                    {enrollment?.enrollmentInfo?.level === 'college' ? 'Prelim' : 'Period 1'}
                  </div>
                </th>
                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200"
                  style={{ fontFamily: 'Poppins', fontWeight: 400 }}>
                  <div className="flex items-center justify-center gap-2">
                    ...
                    {enrollment?.enrollmentInfo?.level === 'college' ? 'Midterm' : 'Period 2'}
                  </div>
                </th>
                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200"
                  style={{ fontFamily: 'Poppins', fontWeight: 400 }}>
                  <div className="flex items-center justify-center gap-2">
                    ...
                    {enrollment?.enrollmentInfo?.level === 'college' ? 'Finals' : 'Period 3'}
                  </div>
                </th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {Object.keys(grades).length === 0 ? (
                <tr>
                  <td colSpan={enrollment?.enrollmentInfo?.level === 'college' ? 5 : 7} className="px-6 py-8 text-center text-gray-500 border-t border-gray-200"
                    style={{ fontFamily: 'Poppins', fontWeight: 400 }}>
                          ...
                          <div className="flex flex-col items-center space-y-2">
                            {safeSubjectGrade.specialStatus ? (
                              <>
                                ...
                                <div className="flex items-center space-x-1">
                                  <div className={`w-2 h-2 ${
                                    safeSubjectGrade.specialStatus === 'INC' ? 'bg-orange-600' :
                                    safeSubjectGrade.specialStatus === 'FA' || safeSubjectGrade.specialStatus === 'FW' ? 'bg-red-600' :
                                    safeSubjectGrade.specialStatus === 'W' ? 'bg-yellow-600' : 'bg-gray-400'
                                  } flex-shrink-0`}></div>
                                  <div className={`text-xs font-medium ${
                                    safeSubjectGrade.specialStatus === 'INC' ? 'text-orange-700' :
                                    safeSubjectGrade.specialStatus === 'FA' || safeSubjectGrade.specialStatus === 'FW' ? 'text-red-700' :
                                    safeSubjectGrade.specialStatus === 'W' ? 'text-yellow-700' : 'text-gray-500'
                                  }`}>
                                    {safeSubjectGrade.specialStatus === 'INC' ? 'Incomplete' :
                                     safeSubjectGrade.specialStatus === 'FA' ? 'Failed (Absent)' :
                                     safeSubjectGrade.specialStatus === 'FW' ? 'Failed (Withdrawn)' :
                                     safeSubjectGrade.specialStatus === 'W' ? 'Withdrawn' : ''}
                                  </div>
                                </div>
                              </>
                            ) : (
                              <>
                                ...
                                {average !== null && typeof average === 'number' && convertToNumericMode(average) !== null && (
                                  <div className="text-xs font-mono text-gray-600"
                                    style={{ fontFamily: 'Poppins', fontWeight: 400 }}>
                                    ({convertToNumericMode(average)!.toFixed(2)})
                                  </div>
                                )}
                                <div className="flex items-center space-x-1">

...
import { Plus, X, Pencil, Palette, Calculator, ArrowRight, ArrowLeft, Sparkle } from '@phosphor-icons/react';
...
export default function SubjectForm({
  ...
  const [formData, setFormData] = useState<SubjectFormData>({
    ...
    gradeLevels: Array.isArray(initialData?.gradeLevels) ? initialData.gradeLevels : (initialData?.gradeLevels ? [initialData.gradeLevels as number] : []),
    ...
    postrequisites: Array.isArray(initialData?.postrequisites) ? initialData.postrequisites : []
  });
  ...
  const loadSubjects = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (isEditing && initialData) {
          const currentSubjectId = (initialData as any).id || `subject-${(initialData.code || '').toUpperCase().replace(/[^A-Z0-9]/g, '')}`;
          subjects = subjects.filter((subject: SubjectData) => subject.id !== currentSubjectId);
        }
  ...
  const handleGenerateDescription = async () => {
    ...
    try {
      ...
      if (response.ok && data.description) {
        ...
        toast.error(data.error || 'Failed to generate description.', { autoClose: 5000 });
        setGeneratingDescription(false);
        setTypewritingText('');
      }
    } catch (error) {
      console.error('Error generating description:', error);
      toast.error('Network error occurred while generating description.', { autoClose: 5000 });
      setGeneratingDescription(false);
      setTypewritingText('');
    }
  };
    ...
    <Card className="w-full max-w-2xl p-6 bg-gray-50 border-0 border-r-0 border-b-0 border-t-5 border-blue-900">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              className={`border-1 shadow-sm border-blue-900 rounded-none uppercase ${errors.code ? 'border-red-500 focus-visible:ring-red-500' : ''}`}
              style={{ fontFamily: 'Poppins', fontWeight: 300 }}
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              disabled={loading}
              className={`border-1 shadow-sm border-blue-900 rounded-none ${errors.name ? 'border-red-500 focus-visible:ring-red-500' : ''}`}
              style={{ fontFamily: 'Poppins', fontWeight: 300 }}
          ...
          <div className="flex gap-4">
            ...
            <div className="flex-1 space-y-2">
              ...
              <Input
                ...
                min={0}
                max={10}
                className={`border-1 shadow-sm border-blue-900 rounded-none w-full ${errors.lectureUnits ? 'border-red-500 focus-visible:ring-red-500' : ''}`}
                style={{ fontFamily: 'Poppins', fontWeight: 300 }}
            ...
            <div className="flex-1 space-y-2">
              ...
              <Input
                ...
                min={0}
                max={10}
                className={`border-1 shadow-sm border-blue-900 rounded-none w-full ${errors.labUnits ? 'border-red-500 focus-visible:ring-red-500' : ''}`}
                style={{ fontFamily: 'Poppins', fontWeight: 300 }}
            ...
            <div className="flex-1 space-y-2">
              ...
              <div className="bg-gray-50 border-1 shadow-sm border-blue-900 p-3 flex items-center justify-center h-9">
                <span
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              value={generatingDescription ? typewritingText : formData.description}
              onChange={(e) => handleInputChange('description', e.target.value)}
              disabled={loading || generatingDescription}
              className={`border-1 shadow-sm border-blue-900 rounded-none flex min-h-[100px] w-full rounded-md bg-white pr-3 py-2 text-base shadow-lg placeholder:text-gray-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-900 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 md:text-sm resize-none ${errors.description ? 'border-red-500 focus-visible:ring-red-500' : ''}`}
              style={{ fontFamily: 'Poppins', fontWeight: 300 }}
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-gray-500" style={{ fontFamily: 'Poppins', fontWeight: 300 }}>
          ...
          {formData.prerequisites.length > 0 && (
            <p className="text-xs text-gray-600" style={{ fontFamily: 'Poppins', fontWeight: 300 }}>
              Selected: {formData.prerequisites.map(id => {
        ...
        <div className="space-y-2">
          ...
          <div className="border border-gray-200 rounded-md p-3 bg-gray-50 max-h-60 overflow-y-auto">
            {loadingSubjects ? (
              ...
              <div className="flex flex-wrap gap-2">
                {availableSubjects.map((subject) => {
                  const isSelected = formData.postrequisites.includes(subject.id);
                  return (
                    <button
                      key={subject.id}
                      type="button"
          ...
          {formData.postrequisites.length > 0 && (
            <p className="text-xs text-gray-600" style={{ fontFamily: 'Poppins', fontWeight: 300 }}>
              Selected: {formData.postrequisites.map(id => {
                const subject = availableSubjects.find(s => s.id === id);
                return subject ? `${subject.code} - ${subject.name}` : id;
              }).join(', ')}
            </p>

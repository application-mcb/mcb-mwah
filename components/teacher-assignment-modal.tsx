...
interface SubjectSet {
  ...
  courseSelections?: { code: string; year: number; semester: 'first-sem' | 'second-sem' }[]; // For college subject sets
  color: string;
  subjects: string[];
  createdAt: string;
  updatedAt: string;
...
export default function TeacherAssignmentModal({
  ...
  const [step, setStep] = useState<'level' | 'grade' | 'subjects' | 'sections'>('level');
  ...
  }, [selectedSubjects, currentSubjectIndex, selectedLevel, selectedGrade, selectedCourse]);
  ...
  const loadSectionsForCourse = async (courseCode: string) => {
    try {
      ...
      if (response.ok) {
        ...
        const filteredSections = (data.sections || []).filter((section: Section) => 
          section.courseId === courseCode || section.grade?.includes(courseCode)
        );
  ...
  const getFilteredSubjects = () => {
    ...
    if (selectedSubjectSetId) {
      ...
      if (selectedSet) {
        filtered = filtered.filter(subject => selectedSet.subjects.includes(subject.id));
      }
    }
          ...
          <div className="relative">
            ...
            <div className="flex justify-between items-start relative">
              ...
              <div
                ...
                style={{
                  width: step === 'level' ? '0%' :
                         step === 'grade' ? 'calc(33% - 24px)' :
                         step === 'subjects' ? 'calc(66% - 48px)' :
                         step === 'sections' ? 'calc(100% - 72px)' :
                         'calc(100% - 72px)'
                }}
              ></div>
              ...
              >
                <div className={`relative w-12 h-12 flex items-center justify-center text-sm font-medium transition-all duration-500 rounded-none ${
                  step === 'level' ? 'bg-blue-900 text-white shadow-lg shadow-blue-900/30' :
                  ['level', 'grade', 'subjects', 'sections'].indexOf(step) > 0 ? 'bg-blue-800 text-white shadow-md' : 'bg-gray-100 text-gray-400 group-hover:bg-gray-200'
                }`}>
                  <ChalkboardSimple size={18} weight="bold" className="transition-all duration-300" />
                  {step === 'level' && (
                ...
                <span className={`text-xs font-medium mt-2 text-center transition-all duration-300 ${
                  step === 'level' ? 'text-blue-900 font-semibold' :
                  ['level', 'grade', 'subjects', 'sections'].indexOf(step) > 0 ? 'text-blue-800' : 'text-gray-400 group-hover:text-gray-600'
              ...
              >
                <div className={`relative w-12 h-12 flex items-center justify-center text-sm font-medium transition-all duration-500 rounded-none ${
                  ...
                  ['level', 'grade', 'subjects', 'sections'].indexOf(step) > 1 ? 'bg-blue-800 text-white shadow-md' : 'bg-gray-100 text-gray-400 group-hover:bg-gray-200'
                }`}>
                ...
                <span className={`text-xs font-medium mt-2 text-center transition-all duration-300 ${
                  ...
                  ['level', 'grade', 'subjects', 'sections'].indexOf(step) > 1 ? 'text-blue-800' : 'text-gray-400 group-hover:text-gray-600'
                }`} style={{ fontFamily: 'Poppins', fontWeight: 400 }}>
              ...
              >
                <div className={`relative w-12 h-12 flex items-center justify-center text-sm font-medium transition-all duration-500 rounded-none ${
                  ...
                  ['level', 'grade', 'subjects', 'sections'].indexOf(step) > 2 ? 'bg-blue-800 text-white shadow-md' : 'bg-gray-100 text-gray-400 group-hover:bg-gray-200'
                }`}>
                ...
                <span className={`text-xs font-medium mt-2 text-center transition-all duration-300 ${
                  ...
                  ['level', 'grade', 'subjects', 'sections'].indexOf(step) > 2 ? 'text-blue-800' : 'text-gray-400 group-hover:text-gray-600'
                }`} style={{ fontFamily: 'Poppins', fontWeight: 400 }}>
                  Subjects
                </span>
              </div>
              ...
              >
                <div className={`relative w-12 h-12 flex items-center justify-center text-sm font-medium transition-all duration-500 rounded-none ${
                  ...
                  ['level', 'grade', 'subjects', 'sections'].indexOf(step) > 3 ? 'bg-blue-800 text-white shadow-md' : 'bg-gray-100 text-gray-400 group-hover:bg-gray-200'
                }`}>
                ...
                <span className={`text-xs font-medium mt-2 text-center transition-all duration-300 ${
                  ...
                  ['level', 'grade', 'subjects', 'sections'].indexOf(step) > 3 ? 'text-blue-800' : 'text-gray-400 group-hover:text-gray-600'
                }`} style={{ fontFamily: 'Poppins', fontWeight: 400 }}>
                  Sections
                </span>
              </div>
        ...
        {step === 'level' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div>
                <div className="flex items-center space-x-3">
                  <div className="w-10 h-10 bg-blue-900 flex items-center justify-center">
                    <ChalkboardSimple size={20} className="text-white" weight="bold" />
                  </div>
                  <div>
                    <h2
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* High School Option */}
              <Card
                className={`group p-6 border-none border-1 shadow-sm bg-gray-50 hover:border-blue-900 cursor-pointer transition-all duration-300 animate-in fade-in slide-in-from-bottom-4 ${
                  selectedLevel === 'high-school' ? 'shadow-lg border-blue-900 bg-blue-50' : ''
                }`}
                  ...
                  <div className="pt-2 border-t border-gray-200">
                    <div className="flex items-center justify-between">
                      ...
                      <div className={`w-4 h-4 border-2 transition-colors ${
                        selectedLevel === 'high-school' ? 'border-blue-900 bg-blue-900' : 'border-gray-300'
                      }`}>
              ...
              <Card
                className={`group p-6 border-none border-1 shadow-sm bg-gray-50 hover:border-blue-900 cursor-pointer transition-all duration-300 animate-in fade-in slide-in-from-bottom-4 ${
                  selectedLevel === 'college' ? 'shadow-lg border-blue-900 bg-blue-50' : ''
                }`}
              ...
              >
                <div className="space-y-4 flex flex-col justify-between h-full">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center space-x-3">
                      <div className="w-10 h-10 bg-blue-900 flex items-center justify-center">
                        <ChalkboardSimple size={20} className="text-white" weight="fill" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <h3
                          className="text-lg font-medium text-gray-900"
                  ...
                  <p
                    ...
                    style={{ fontFamily: 'Poppins', fontWeight: 300 }}
                  >
                    Assign subjects to college courses with year levels and semesters
                  </p>
                  ...
                  <div className="pt-2 border-t border-gray-200">
                    <div className="flex items-center justify-between">
                      ...
                      <div className={`w-4 h-4 border-2 transition-colors ${
                        selectedLevel === 'college' ? 'border-blue-900 bg-blue-900' : 'border-gray-300'
                      }`}>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h2
                      ...
                      style={{ fontFamily: 'Poppins', fontWeight: 400 }}
                    >
                      {selectedLevel === 'college' ? 'Select Course' : 'Select Grade Level'}
                    </h2>
                    <p
                      className="text-xs text-gray-600"
                      style={{ fontFamily: 'Poppins', fontWeight: 300 }}
                    >
                      {selectedLevel === 'college' ? 'Choose the course for subject assignment' : 'Choose the grade level for subject assignment'}
                    </p>
                  </div>
                </div>
              </div>
            ...
            {selectedLevel === 'college' ? (
              courses.length === 0 ? (
                <Card className="p-12 text-center border-none bg-gray-50 border-1 shadow-sm border-blue-900">
                  <ChalkboardSimple size={48} className="mx-auto text-gray-400 mb-4" weight="duotone" />
                  <h3
                  ...
                  <p
                    ...
                    style={{ fontFamily: 'Poppins', fontWeight: 300 }}
                  >
                    There are currently no college courses available for subject assignment. Please contact your registrar or try again later.
                  </p>
                </Card>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {courses.map((course, index) => (
                    <Card
                      key={course.code}
                      className={`group p-6 border-none border-1 shadow-sm bg-gray-50 hover:border-blue-900 cursor-pointer animate-in fade-in slide-in-from-bottom-4`}
                      style={{
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex items-center space-x-3">
                            <div className="w-10 h-10 flex items-center justify-center bg-white">
                              <ChalkboardSimple size={20} weight="fill" style={{ color: getColorValue(course.color) }} />
                            </div>
                            <div className="flex-1 min-w-0">
                              <h3
            ...
            <div className="mb-6">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 border-2 border-gray-300 rounded-none focus:outline-none focus:border-blue-900 transition-colors"
                  style={{ fontFamily: 'Poppins', fontWeight: 300, color: '#000' }}
                />
                {searchQuery && (
                  <button

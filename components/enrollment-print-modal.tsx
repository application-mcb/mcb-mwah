...
import { User as UserIcon, GraduationCap as GraduationCapIcon } from '@phosphor-icons/react';
...
const EnrollmentPrintModal: React.FC<EnrollmentPrintModalProps> = ({
  ...
  const formatSignatureName = (firstName?: string, middleName?: string, lastName?: string, nameExtension?: string) => {
    if (!lastName && !firstName) return 'N/A';
  ...
  const getColorValue = (colorClass: string) => {
    ...
    const colorMap: Record<string, string> = {
      ...
      // Cyan variants
      'cyan-500': '#06b6d4', 'cyan-600': '#0891b2', 'cyan-700': '#0e7490', 'cyan-800': '#155e75', 'cyan-900': '#164e63',
      // Lime variants
      'lime-500': '#84cc16', 'lime-600': '#65a30d', 'lime-700': '#365314', 'lime-800': '#3f6212', 'lime-900': '#365314',
      // Emerald variants
      ...
      // Fuchsia variants
      'fuchsia-500': '#d946ef', 'fuchsia-600': '#c026d3', 'fuchsia-700': '#a21caf', 'fuchsia-800': '#86198f', 'fuchsia-900': '#701a75',
      // Rose variants
  ...
  const getSelectedSubjectsData = () => {
    ...
    const result = selectedSubjects.map(subjectId => {
      ...
      <div className="print-document p-3">
        ...
        <div className="print-header mb-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              />
              <div>
                <h1 className="text-md font-medium text-gray-900" style={{ fontFamily: 'Poppins', fontWeight: 500 }}>
                  Marian College of Baliuag, Inc.
                </h1>
                <p className="text-xs text-gray-600 font-mono" style={{ fontWeight: 400 }}>
                  908 Gil Carlos St. San Jose, Baliwag, Bulacan
                </p>
                <p className="text-xs font-mono text-gray-600 ">Enrollment Date: {new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
                ...
                <label className="text-sm font-medium text-gray-700" style={{ fontFamily: 'Poppins', fontWeight: 500 }}>
                    OR Number
                </label>
                <p className="w-32 px-2 py-1 text-sm border border-gray-300 bg-gray-50 font-mono" style={{fontWeight: 400 }}>
                    {orNumber || 'XXXXX'}
                </p>
                </div>
          ...
          <div>
            <div className="grid grid-cols-3 gap-4 mt-4 print-section">
              {getSelectedSubjectsData().map((subject, index) => {
                ...
                console.log(`Subject: ${subject.name}, Color: ${subject.color}, Hex: ${getColorValue(subject.color)}`);
                return (
                  <div key={subject.id} className="flex items-center justify-between p-3 bg-gray-50 border border-gray-200">
                    <div className="flex items-center gap-2">
                      <div 
                      ...
                      ></div>
                      <span className="text-xs text-gray-900 font-mono uppercase" style={{ fontWeight: 400 }}>
                        {subject.code} 
                      </span>
                    </div>
                    <span className="text-xs text-gray-600 font-medium font-mono" style={{fontWeight: 400 }}>
        ...
        <div className="print-section">
          <div className="flex justify-between"  >
            ...
            <p className="font-regular text-xs border-t-1 w-[200px] text-center border-black pt-2 mt-8">{registrarName || 'Registrar'}</p>
          </div>
        </div>
      </div>
    </Print>

...
export default function RegistrarOverview({ registrarUid }: RegistrarOverviewProps) {
  ...
  useEffect(() => {
    ...
    const studentsInterval = setInterval(() => {
      if (stats.recentEnrollments.length > 0) {
        setStudentsCarouselIndex((prev) => (prev + 1) % Math.ceil(stats.recentEnrollments.length / 3));
      }
    }, 4000);

    const teachersInterval = setInterval(() => {
      if (stats.recentTeachers.length > 0) {
        setTeachersCarouselIndex((prev) => (prev + 1) % Math.ceil(stats.recentTeachers.length / 3));
      }
    }, 5000);

    const gradesInterval = setInterval(() => {
      if (stats.recentGrades.length > 0) {
        setGradesCarouselIndex((prev) => (prev + 1) % Math.ceil(stats.recentGrades.length / 3));
      }
    }, 4500);
  ...
  }, [navigationItems.length, stats.recentEnrollments.length, stats.recentTeachers.length, stats.recentGrades.length]);
  ...
  const getGradeColorByLevel = (gradeLevel: number, grades: any): string => {
    ...
    const gradeEntry = Object.entries(grades).find(([id, grade]: [string, any]) => 
  ...
  const getRelativeTime = (dateInput: any): { text: string; color: string } => {
    try {
      ...
      if (diffInMinutes < 1) {
        ...
        return { text: `${diffInMinutes} minute${diffInMinutes === 1 ? '' : 's'} ago`, color: 'green-600' };
      } else if (diffInHours < 24) {
        return { text: `${diffInHours} hour${diffInHours === 1 ? '' : 's'} ago`, color: 'green-500' };
      } else if (diffInDays === 1) {
        return { text: 'Yesterday', color: 'yellow-500' };
      } else if (diffInDays < 7) {
        return { text: `${diffInDays} day${diffInDays === 1 ? '' : 's'} ago`, color: 'yellow-600' };
      } else {
  ...
  useEffect(() => {
    const fetchOverviewData = async () => {
      try {
        ...
        const [sectionsResponse, gradesResponse, coursesResponse] = await Promise.all([
          fetch('/api/sections'),
          fetch('/api/grades'),
          fetch('/api/courses')
        ]);
        ...
        const pendingEnrollments = enrollments.filter((e: any) => e.enrollmentInfo?.status === 'pending').length;
        const approvedEnrollments = enrollments.filter((e: any) => e.enrollmentInfo?.status === 'approved').length;
        const rejectedEnrollments = enrollments.filter((e: any) => e.enrollmentInfo?.status === 'rejected').length;
        ...
        const recentEnrollments = enrollments
          .filter((enrollment: any) => enrollment.enrollmentInfo?.status !== 'enrolled')
          ...
          teachers
            ...
            .map(async (teacher: any) => {
              try {
                ...
                const assignmentResponse = await fetch(`/api/teacher-assignments?teacherId=${encodeURIComponent(teacher.id)}`);
                let assignmentCounts = { subjects: 0, sections: 0 };
                
                if (assignmentResponse.ok) {
                  const assignmentData = await assignmentResponse.json();
                  const assignments: Record<string, string[]> = assignmentData.assignments || {};
        ...
        const recentGrades = gradesApiData.grades
          ?.sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
          .slice(0, 5)
          .map((grade: any) => {
            // Count sections for this grade
            const sectionsForGrade = sectionsData.sections?.filter((section: any) => section.gradeId === grade.id) || [];
  ...
  const RecentEnrollmentsSkeleton = () => (
    <div className="space-y-4">
      <h3 className="text-lg font-medium text-gray-900 mb-4" style={{ fontFamily: 'Poppins', fontWeight: 400 }}>
        Recent Enrollments
      </h3>
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-light text-gray-900" style={{ fontFamily: 'Poppins', fontWeight: 300 }}>
              Registrar Overview
            </h1>
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="px-3 py-1 bg-gray-100 border border-gray-200">
            <span className="text-sm text-gray-700 font-mono" style={{ fontFamily: 'Poppins', fontWeight: 300 }}>
      ...
      <div className="bg-white p-6 border border-gray-200 shadow-lg">
        ...
        <div className="relative overflow-hidden">
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {[...Array(3)].map((_, i) => (
                <div key={i} className="p-4 border-none bg-gray-50 border-1 shadow-sm border-blue-900 animate-pulse">
                      ...
                      >
                        <div className="flex items-center space-x-4">
                          ...
                          <div className="w-12 h-12 bg-blue-900 flex items-center justify-center border-2 border-gray-300">
                            <item.icon size={20} className="text-white" weight="fill" />
                          </div>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center space-x-2 mt-1">
                              <div className={`w-3 h-3 bg-${item.color}`}></div>
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="relative overflow-hidden">
            {loading ? (
              ...
              <div
                className="flex transition-transform duration-500 ease-in-out"
                style={{ transform: `translateX(-${studentsCarouselIndex * 100}%)` }}
              >
                {Array.from({ length: Math.ceil(stats.recentEnrollments.length / 3) }).map((_, groupIndex) => (
                  <div key={groupIndex} className="w-full flex-shrink-0 animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: `${groupIndex * 150}ms` }}>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {stats.recentEnrollments.slice(groupIndex * 3, (groupIndex + 1) * 3).map((enrollment, studentIndex) => {
                    return (
                    <div
                    ...
                    >
                      ...
                      <div className="flex items-center space-x-4">
                        ...
                        <div className="w-12 h-12 bg-blue-900 flex items-center justify-center border-2 border-gray-300 rounded-full ">
                          {(enrollment.profile?.photoURL || enrollment.personalInfo?.photoURL) ? (
                            <img
                              src={enrollment.profile?.photoURL || enrollment.personalInfo?.photoURL}
                            ...
                            <span className="text-white text-sm font-medium" style={{ fontFamily: 'Poppins', fontWeight: 500 }}>
                              {getStudentInitials(
                                enrollment.personalInfo?.firstName || 'Student',
                                enrollment.personalInfo?.lastName || 'Name'
                              )}
                            </span>
                          )}
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center space-x-2">
                            <div
                              ...
                              style={{ backgroundColor: getGradeColorByLevel(parseInt(enrollment.enrollmentInfo?.gradeLevel || '0'), stats.grades) }}
                            ></div>
                          ...
                          <div className="flex items-center space-x-2 mt-1">
                            <div className={`w-3 h-3 border-2 ${
                              enrollment.enrollmentInfo?.status === 'approved' ? 'border-green-600 bg-green-600' :
                              enrollment.enrollmentInfo?.status === 'pending' ? 'border-yellow-600 bg-yellow-600' :
                              enrollment.enrollmentInfo?.status === 'enrolled' ? 'border-blue-600 bg-blue-600' :
                          ...
                          <div className="flex items-center space-x-2 mt-1">
                            {(() => {
                              ...
                              const enrollmentDate = enrollment.submittedAt || enrollment.createdAt || new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString();
                              const relativeTime = getRelativeTime(enrollmentDate);
                              return (
                                <>
                                  <div className={`w-3 h-3 border-2 bg-${relativeTime.color} border-${relativeTime.color}`}></div>
              ...
              {stats.recentEnrollments.length > 3 && (
                <div className="flex justify-center mt-4 space-x-2">
                  {Array.from({ length: Math.ceil(stats.recentEnrollments.length / 3) }).map((_, index) => (
                    <button
                      key={index}
                      onClick={() => setStudentsCarouselIndex(index)}
                      className={`w-2 h-2 rounded-full transition-colors duration-200 ${
                        index === studentsCarouselIndex ? 'bg-blue-900' : 'bg-gray-300'
                      }`}
                    />
                  ))}
                </div>
      ...
      <div className="bg-white p-6 border border-gray-200 shadow-lg ">
        ...
        <div className="relative overflow-hidden">
          {loading ? (
            ...
            <div
              className="flex transition-transform duration-500 ease-in-out"
              style={{ transform: `translateX(-${teachersCarouselIndex * 100}%)` }}
            >
              {Array.from({ length: Math.ceil(stats.recentTeachers.length / 3) }).map((_, groupIndex) => (
                <div key={groupIndex} className="w-full flex-shrink-0 animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: `${groupIndex * 150}ms` }}>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {stats.recentTeachers.slice(groupIndex * 3, (groupIndex + 1) * 3).map((teacher, teacherIndex) => {
                      return (
                        <div
                        ...
                        >
                          ...
                          <div className="flex items-center space-x-4">
                            ...
                            <div className="flex-1 min-w-0">
                              {/* Full Name */}
                              <h3 className="text-sm font-medium text-gray-900 truncate" style={{ fontFamily: 'Poppins', fontWeight: 500 }}>
                                {teacher.firstName} {teacher.middleName && `${teacher.middleName.charAt(0)}. `}{teacher.lastName}{teacher.extension && ` ${teacher.extension}`}
                              </h3>
                              ...
                              <div className="flex items-center space-x-2">
                                ...
                                <p className="text-xs text-black truncate font-mono">
                                  {teacher.assignmentCounts?.subjects || 0} Subject{(teacher.assignmentCounts?.subjects || 0) !== 1 ? 's' : ''}
                                </p>
                              </div>
                              ...
                              <div className="flex items-center space-x-2 mt-1">
                                ...
                                <p className="text-xs text-black truncate font-mono">
                                  {teacher.assignmentCounts?.sections || 0} Section{(teacher.assignmentCounts?.sections || 0) !== 1 ? 's' : ''}
                                </p>
                              </div>
                            </div>
                          </div>
          ...
          {stats.recentTeachers.length > 3 && (
            <div className="flex justify-center mt-4 space-x-2">
              {Array.from({ length: Math.ceil(stats.recentTeachers.length / 3) }).map((_, index) => (
                <button
                  key={index}
                  onClick={() => setTeachersCarouselIndex(index)}
                  className={`w-2 h-2 rounded-full transition-colors duration-200 ${
                    index === teachersCarouselIndex ? 'bg-blue-900' : 'bg-gray-300'
                  }`}
                />
              ))}
            </div>
            ...
            <div className="flex justify-center mt-4 space-x-2">
              {Array.from({ length: Math.ceil((stats.recentGrades.length + stats.recentCourses.length) / 3) }).map((_, index) => (
                <button
                  ...
                  className={`w-2 h-2 rounded-full transition-colors duration-200 ${
                    index === gradesCarouselIndex ? 'bg-blue-900' : 'bg-gray-300'
                  }`}
                />
              ))}
            </div>

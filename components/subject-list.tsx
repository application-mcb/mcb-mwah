    ...
    if (selectedCourses.length > 0) {
      filtered = filtered.filter((subject) => {
        if (subject.courseCodes && Array.isArray(subject.courseCodes)) {
          return selectedCourses.some(courseCode => subject.courseCodes.includes(courseCode));
        }
        return false;
      });
    }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 border border-gray-200">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            <div className="flex-1 max-w-md">
              <Input
                ...
                className="pr-4 py-2 border-1 shadow-sm border-blue-900 rounded-none"
          ...
          <div className="flex items-center space-x-3">
            {(searchQuery || selectedGradeLevel || selectedCourses.length > 0) && (
              <Button
                variant="ghost"
      ...
      {filteredSubjects.length === 0 ? (
        <Card className="w-full max-w-md mx-auto p-8 border-none text-center bg-gray-50 border-1 shadow-sm border-blue-900">
          ...
          <p
            className="text-blue-900 text-sm text-justify w-full border-1 shadow-sm border-blue-900 p-3 bg-blue-100 mb-4"
            style={{ fontFamily: 'Poppins', fontWeight: 300 }}
          >
              ...
              <div key={key} className="animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: `${keyIndex * 150}ms` }}>
                ...
                <div className="mb-6">
                  <div className="flex items-center space-x-3 mb-2">
                    ...
                    <div
                      ...
                      style={{ backgroundColor: getBgColor(gradeColor) }}
                    ></div>
                    <hr className="w-full border-1 shadow-sm" style={{ borderColor: getBgColor(gradeColor) }} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 ml-7">
                  {gradeSubjects && gradeSubjects.length > 0 ? gradeSubjects.map((subject, subjectIndex) => (
                    <Card
                      key={`${key}-${subject.id}`}
                      className={`group p-6 border-none hover:shadow-lg hover:-translate-y-2 transition-all duration-300 ease-in-out border-1 shadow-sm bg-${subject.color} text-white transform hover:scale-105 animate-in fade-in slide-in-from-bottom-4 h-full flex flex-col`}
                      style={{

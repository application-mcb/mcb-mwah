...
export class EnrollmentDatabase {
  ...
  static async getSubjectsByIds(subjectIds: string[]): Promise<{ success: boolean; subjects?: any[]; error?: string }> {
    try {
      ...
      for (let i = 0; i < subjectIds.length; i += batchSize) {
        ...
        const q = query(subjectsRef, where('__name__', 'in', batch.map(id => doc(db, 'subjects', id))));

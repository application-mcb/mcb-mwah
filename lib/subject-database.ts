...
export class SubjectDatabase {
  ...
  static async createSubject(subjectData: CreateSubjectData): Promise<SubjectData> {
    try {
      ...
      if ((!subjectData.gradeLevels || subjectData.gradeLevels.length === 0) &&
          ...
          (!subjectData.courseSelections || subjectData.courseSelections.length === 0)) {
      ...
      if (finalCourseCodes.length === 0 && subjectData.courseSelections && subjectData.courseSelections.length > 0) {
        finalCourseCodes = Array.from(new Set(subjectData.courseSelections.map((sel: any) => sel.code)));
      }
  ...
  static async updateSubject(id: string, updateData: Partial<CreateSubjectData>): Promise<SubjectData | null> {
    try {
      ...
      if (updateData.courseSelections !== undefined) {
        ...
        for (const selection of updateData.courseSelections) {
          ...
          if (!selection.year || selection.year < 1 || selection.year > 4) {
            throw new Error('Each course selection must have a valid year (1-4)');
          }
          if (!selection.semester || !['first-sem', 'second-sem'].includes(selection.semester)) {
            throw new Error('Each course selection must have a valid semester (first-sem or second-sem)');
          }
        }
      }

      // Validate that at least one grade level, course code, or course selection is provided
      const hasGradeLevels = updateData.gradeLevels !== undefined && updateData.gradeLevels.length > 0;
      const hasCourseCodes = updateData.courseCodes !== undefined && updateData.courseCodes.length > 0;
      const hasCourseSelections = updateData.courseSelections !== undefined && updateData.courseSelections.length > 0;
      
      // If any of these fields are being updated, at least one must have a value
      if (updateData.gradeLevels !== undefined || updateData.courseCodes !== undefined || updateData.courseSelections !== undefined) {
      ...
      if (updateData.courseSelections !== undefined) {
        if (updateData.courseCodes === undefined || (Array.isArray(updateData.courseCodes) && updateData.courseCodes.length === 0)) {
          if (updateData.courseSelections.length > 0) {
            finalCourseCodes = Array.from(new Set(updateData.courseSelections.map((sel: any) => sel.code)));
          } else {
            finalCourseCodes = [];
          }
        }
      ...
      const updatePayload: any = {
        ...updateData,
        ...(finalCourseCodes !== undefined && { courseCodes: finalCourseCodes }),
        updatedAt: serverTimestamp(),
      };
